...
def main(page: ft.Page):
    ...
    def generate_profile(e):
        try:
            # Validate required fields (First name, Last name, Age, and Student ID)
            if not first_name.value or not last_name.value:
                show_error("Please fill in both First Name and Last Name!")
                return
            
            if not age.value:
                show_error("Please fill in the Age!")
                return
            
            if not student_id.value:
                show_error("Please fill in the Student ID!")
                return
            
            # Calculate birth year
            current_year = datetime.now().year
            birth_year = current_year - int(age.value)
            graduation_year = current_year + (5 - int(year_level.value[0]) if year_level.value else 4)
            ...
            profile_content = ft.Column([
                ...
                ft.Text(f"🆔 Student ID: {student_id.value}", size=16),
            ...
            show_error("Please enter a valid age (number only)!")  # Catch invalid age
